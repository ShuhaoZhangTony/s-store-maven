package frontend.edu.brown.markov;
...
import frontend.edu.brown.graphs.exceptions.InvalidGraphElementException;
import frontend.edu.brown.hstore.estimators.DynamicTransactionEstimate;
...
//        READ_ONLY       (false, 0.0f),
    ...
    public MarkovVertex() {
        ...
        super();
        this.probabilities = new float[Probability.values().length][];
    }
     ...
     * @param type
     */
    public MarkovVertex(Statement catalog_stmt, Type type) {
     ...
     */
    public MarkovVertex(Statement catalog_stmt, Type type, int query_instance_index, PartitionSet partitions, PartitionSet past_partitions) {
        super(catalog_stmt);
        this.type = type;
    ...
    private void init() {
        ...
        for (Probability ptype : Probability.values()) {
...
//                    this.query_instance_index == v.query_instance_index);
     ...
     * @return
     */
    private float getSpecificProbability(Probability ptype, int partition) {
     ...
     * @param probability
     */
    private void addToProbability(Probability ptype, int partition, float probability) {
     ...
     * @param probability
     */
    private void setProbability(Probability ptype, int partition, float probability) {
        if (trace.val)
...
//    }
    ...
    public String debug() {
        ...
        // if (true || this.isQueryVertex()) {
            ...
            header.add(" ");
            Probability ptypes[] = Probability.values();
            for (Probability type : ptypes) {
            ...
            int num_partitions = this.probabilities[Probability.WRITE.ordinal()].length;
