 ...
 * as an unscaled (unscaled means no trailing 0s) fixed point {@link BigInteger BigInteger} and a separate
 * scale value. An exception (either {@link RuntimeException RuntimeException} or
 * {@link IOException IOException}) if a <code>BigDecimal</code> with a scale > 12 or precision greater then
 * 38 is used. {@link BigDecimal#setScale(int) BigDecimal.setScale(int)} can be used to reduce the scale of
...
public class VoltDecimalHelper {
    ...
    /**
     * Serialize the null decimal sigil to a the provided {@link FastSerializer FastSerializer}
     ...
     * @param out {@link FastSerializer FastSerializer} to serialize the <code>BigDecimal</code> to
     ...
     * Serialize the {@link BigDecimal BigDecimal} to Volt's fixed precision and scale 16-byte format.
     * @param bd {@link BigDecimal BigDecimal} to serialize
     * @param buf {@link ByteBuffer ByteBuffer} to serialize the <code>BigDecimal</code> to
     ...
     * @param in {@link FastDeserializer FastDeserializer} to read from
    ...
    public static void main(String args[]) throws Exception {
       ...
       VoltDecimalHelper.serializeBigDecimal(bd, buffer);
        ...
        buffer.flip();
        System.out.println(VoltDecimalHelper.deserializeBigDecimal(buffer));
        System.out.println("----");
        FastSerializer fs = new FastSerializer();
        bd = new BigDecimal("-23325.23425");
       VoltDecimalHelper.serializeBigDecimal(bd, fs);
        ...
        System.out.println(VoltDecimalHelper.deserializeBigDecimal(fds));
