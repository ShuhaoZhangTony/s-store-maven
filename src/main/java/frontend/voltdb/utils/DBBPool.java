...
/**
 * A pool of {@link ByteBuffer ByteBuffers} that are
 * allocated with
 * {@link ByteBuffer#allocateDirect(int) * ByteBuffer.allocateDirect}.
...
//            }
     ...
     * all {@link ByteBuffer ByteBuffer}s as {@link java.nio.HeapByteBuffer HeapByteBuffer}s that are not pooled.
     */
     ...
     *        all buffers as non-pooled heap {@link ByteBuffer ByteBuffer}s
    ...
    public DBBPool(boolean allocateOnHeap, int maxArenaSizes[], boolean loadNativeLib) {
        if (loadNativeLib) {
            foundNativeSupport = frontend.voltdb.EELibraryLoader.loadExecutionEngineLibrary(false);
        } else {
